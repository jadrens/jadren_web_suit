...
import { blogRejectionEmail } from "./emails/blog-rejection";
import { blogApprovalEmail } from "./emails/blog-approval";
import { blogDeletionEmail } from "./emails/blog-deletion";
...
async function sendAuditedEmail(input: {
  ...
  category: "verification" | "reminder" | "blog_rejection" | "blog_approval" | "blog_deletion";
  recipient: string;
  subject: string;
  text: string;
  html: string;
...
export async function sendBlogRejectionEmail(input: {
  email: string;
  userId: string;
  pendingId: string;
  title: string;
  ...
  const content = blogRejectionEmail(input);
  return sendAuditedEmail({
    userId: input.userId,
    reminderId: null,
    category: "blog_rejection",
    recipient: input.email,
    subject: content.subject,
    html: content.html,
    text: content.text,
    idempotencyKey: `blog-rejection/${input.pendingId}`,
    metadata: {
...
export async function sendBlogApprovalEmail(input: {
  email: string;
  userId: string;
  pendingId: string;
  title: string;
  ...
  const content = blogApprovalEmail(input);
  return sendAuditedEmail({
    userId: input.userId,
    reminderId: null,
    ...
    html: content.html,
    text: content.text,
    idempotencyKey: `blog-approval/${input.pendingId}`,
    metadata: {
      pendingId: input.pendingId,
      locale: input.locale,
      slug: input.slug,
...
export async function sendBlogDeletionEmail(input: {
  email: string;
  userId: string;
  postId: string;
  title: string;
  ...
  const content = blogDeletionEmail(input);
  return sendAuditedEmail({
    userId: input.userId,
    reminderId: null,
    category: "blog_deletion",
    recipient: input.email,
    subject: content.subject,
    html: content.html,
    text: content.text,
    idempotencyKey: `blog-deletion/${input.postId}`,
    metadata: {
      postId: input.postId,
      locale: input.locale,
      slug: input.slug,
