...
import { getLlmModels, getLlmProfiles, LlmClient, type LlmMessage, type LlmModelProfile, type LlmProfile } from "@lib/llm";
import { useI18n } from "@lib/i18n/app";

interface VocabularyProposal { proposalId: string; collectionId: string; word: string; prompt: string; example: string }
...
export default function AiCenterClient() {
  ...
  const [models, setModels] = useState<LlmModelProfile[]>([]); const [providers, setProviders] = useState<LlmProfile[]>([]); const [modelId, setModelId] = useState(""); const [usages, setUsages] = useState<VocabularyUsage[]>([]); const [collections, setCollections] = useState<VocabularyCollectionSummary[]>([]); const [collectionId, setCollectionId] = useState(""); const [collectionDialogOpen, setCollectionDialogOpen] = useState(false);
  ...
  useEffect(() => { if (status === "authenticated" && user?.status === 1) { const timer = window.setTimeout(() => void vocabularyPracticeApi.list().then((response) => { setUsages(response.usages); setCollections(response.collections); setCollectionId(current => response.collections.some(item => item.collectionId === current) ? current : (response.collections[0]?.collectionId || "")); }).catch(() => undefined), 0); return () => window.clearTimeout(timer); } }, [status, user?.status, user?.userId]);
  ...
  async function send(event: FormEvent) {
    event.preventDefault(); const request = input.trim(); if (!request || !selected || !provider || !collectionId || running) return;
    ...
    try {
      ...
      for await (const streamEvent of client.stream({
        ...
        tools: [{ name: "read_user_vocabulary", description: "Read the signed-in user's vocabulary usages, learning statistics, and optionally their five most recent attempts. Use an empty query to read all data.", parameters: { type: "object", properties: { query: { type: "string", description: "Case-insensitive word or meaning search; use an empty string for all usages" }, include_attempts: { type: "boolean" } }, required: ["query", "include_attempts"], additionalProperties: false }, execute: (args) => {
          ...
          const proposal = { proposalId: crypto.randomUUID(), collectionId, word, prompt: meaning, example }; pendingProposalsRef.current = [...pendingProposalsRef.current, proposal]; toolCount++;
          ...
          let added = true;
          try { const saved = await vocabularyPracticeApi.createUsage({ collectionId: proposal.collectionId, word: proposal.word, prompt: proposal.prompt }); setUsages((current) => [...current, saved.usage]); }
