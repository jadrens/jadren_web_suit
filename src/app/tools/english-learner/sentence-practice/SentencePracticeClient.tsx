...
function sentenceCandidatePool(usages: VocabularyUsage[], limit = 40) {
  return [...usages].sort((a, b) => {
    const aPracticed = a.attempts.length > 0 || Boolean(a.lastLearnTime); const bPracticed = b.attempts.length > 0 || Boolean(b.lastLearnTime);
    if (aPracticed !== bPracticed) return aPracticed ? 1 : -1;
    const aTime = a.lastLearnTime ? new Date(a.lastLearnTime).getTime() : 0; const bTime = b.lastLearnTime ? new Date(b.lastLearnTime).getTime() : 0;
    return aTime - bTime || (a.correct - a.wrong) - (b.correct - b.wrong) || a.word.localeCompare(b.word);
  }).slice(0, limit);
}

const copyByLocale = {
  en: {
    title: "Vocabulary Sentence Practice", subtitle: "Learn each meaning through AI-guided sentence writing.", login: "Sign in to use sentence practice.", signIn: "Sign in", verify: "Verify your email before using sentence practice.", llm: "Configure an LLM Provider and Model in Settings first.", settings: "Open Settings", empty: "This collection has no sentence-practice usages yet.", noCollections: "Create a collection before adding sentence-practice usages.", manage: "Manage vocabulary in User Data", generate: "Generate exercise", generating: "Selecting and generating…", answer: "Write your sentence", submit: "Check sentence", checking: "Checking…", example: "Example", correct: "Correct", wrong: "Needs work", corrected: "Suggested sentence", loadFailed: "Unable to load vocabulary.", aiFailed: "The AI did not complete the required tool workflow.", gradeFailed: "The AI could not grade this attempt.", saveGradeFailed: "Grading succeeded, but the attempt could not be saved.", model: "Model", collection: "Collection", newCollection: "+ New…",
  },
  zh: {
    title: "单词造句练习", subtitle: "针对每一种词汇用法进行 AI 造句训练。", login: "登录后才能使用单词造句练习。", signIn: "去登录", verify: "请先验证邮箱再使用造句练习。", llm: "请先在设置中配置 LLM Provider 和 Model。", settings: "打开设置", empty: "当前收藏夹还没有造句练习词义。", noCollections: "请先新建收藏夹，再添加造句练习词义。", manage: "前往 User Data 管理词汇", generate: "生成题目", generating: "正在选择并出题…", answer: "写下你的英文句子", submit: "提交判题", checking: "正在判题…", example: "例句", correct: "正确", wrong: "需要修改", corrected: "建议句子", loadFailed: "无法加载词汇。", aiFailed: "AI 未完成要求的工具流程。", gradeFailed: "AI 无法完成本次判题。", saveGradeFailed: "判题已完成，但本次记录保存失败。", model: "模型", collection: "收藏夹", newCollection: "+ 新建…",
  },
};
...
export default function SentencePracticeClient() {
  ...
  const [usages, setUsages] = useState<VocabularyUsage[]>([]); const [collections, setCollections] = useState<VocabularyCollectionSummary[]>([]); const [collectionId, setCollectionId] = useState(""); const [collectionDialogOpen, setCollectionDialogOpen] = useState(false); const [models, setModels] = useState<LlmModelProfile[]>([]); const [providers, setProviders] = useState<LlmProfile[]>([]); const [modelId, setModelId] = useState("");
  ...
  const practiceUsages = usages.filter((item) => item.collectionId === collectionId);
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const data = await vocabularyPracticeApi.list(); setUsages(data.usages); setCollections(data.collections); setCollectionId(current => data.collections.some(item => item.collectionId === current) ? current : (data.collections[0]?.collectionId || "")); } catch { setError(copy.loadFailed); } finally { setLoading(false); } }, [copy.loadFailed]);
  ...
  async function generateExercise() {
    if (!selected || !provider || !practiceUsages.length) return; setBusy(true); setError(""); setExercise(null); setGrade(null); setAnswer("");
    ...
    const candidatePool = sentenceCandidatePool(practiceUsages); const expected = Math.min(5, candidatePool.length);
    try {
      const client = new LlmClient({ profile: provider, model: selected.modelId, maxToolRounds: 5 });
      for await (const event of client.stream({
        messages: [{ role: "user", content: `Choose the best vocabulary practice now from this prioritized JSON data. Unpracticed meanings appear first:\n${JSON.stringify(candidatePool)}` }],
        ...
        tools: [
          { name: "select_candidates", description: `Select exactly ${expected} best items for the learner to review. Prefer meanings with no attempts.`, parameters: { type: "object", properties: { usage_ids: { type: "array", items: { type: "string" }, minItems: expected, maxItems: expected } }, required: ["usage_ids"], additionalProperties: false }, execute: (args) => { const ids = Array.isArray(args.usage_ids) ? args.usage_ids.map(String) : []; if (ids.length !== expected || new Set(ids).size !== expected || ids.some((id) => !candidatePool.some((item) => item.usageId === id))) throw new Error("Invalid candidate selection"); candidateIds = ids; return { candidates: candidatePool.filter((item) => ids.includes(item.usageId)) }; } },
  ...
  return <div className="page-below-navbar flex flex-col"><Box component="main" sx={{ flex: 1, px: { xs: 2, md: 3 }, py: 5 }}><Box sx={{ maxWidth: 760, mx: "auto" }}>
    ...
    <CollectionDialog open={collectionDialogOpen} locale={locale} onClose={() => setCollectionDialogOpen(false)} onCreated={collection => { setCollections(current => [...current, { collectionId: collection.collectionId, name: collection.name }]); setCollectionId(collection.collectionId); }} />
  </Box></Box><Footer /></div>;
}
