  ...
  async function generateExercise() {
    ...
    try {
      ...
      for await (const event of client.stream({
        ...
        tools: [
          { name: "select_candidates", description: `Select exactly ${expected} best items for the learner to review.`, parameters: { type: "object", properties: { usage_ids: { type: "array", items: { type: "string" }, minItems: expected, maxItems: expected } }, required: ["usage_ids"], additionalProperties: false }, execute: (args) => { const ids = Array.isArray(args.usage_ids) ? args.usage_ids.map(String) : []; if (ids.length !== expected || new Set(ids).size !== expected || ids.some((id) => !practiceUsages.some((item) => item.usageId === id))) throw new Error("Invalid candidate selection"); candidateIds = ids; return { candidates: practiceUsages.filter((item) => ids.includes(item.usageId)) }; } },
