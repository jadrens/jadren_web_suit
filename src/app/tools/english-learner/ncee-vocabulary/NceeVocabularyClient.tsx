...
import { nceeVocabularyTranslations } from "@lib/i18n/ncee-vocabulary";
...
interface Exercise { id: number; english: string; phonetic: string; phonetics?: Array<{ accent: "uk" | "us" | "other"; text: string }>; chinese: string; examples?: Array<{ en: string; zh: string }>; hintIndexes: number[]; duplicateCount: number; exerciseKind?: "meanings" | "dictionary"; meanings?: DrillMeaning[]; meaningHints?: string[]; sourceDataset?: string; quizDataset?: string }
...
interface StatDelta { reviews?: import("@lib/vocabulary-practice/collection-memory").ReviewEvent[]; lastReviewedAt?: string; collectionId: string; dataset: string; sourceWordId: number; appearances: number; correct: number; wrong: number }
...
function exerciseMeanings(exercise: Exercise) { return (exercise.meanings?.length ? exercise.meanings : [{ text: exercise.chinese, partOfSpeech: "other" as const }]).slice(0, 4); }
...
function MeaningOptionButtons({ copy, firstLetterHint, pauseAfterCorrect, onFirstLetterHint, onPauseAfterCorrect }: { copy: typeof nceeVocabularyTranslations.en; firstLetterHint: boolean; pauseAfterCorrect: boolean; onFirstLetterHint: () => void; onPauseAfterCorrect: () => void }) {
  return <><Tooltip title={copy.firstLetterHintDescription}><IconButton size="small" aria-label={copy.firstLetterHint} aria-pressed={firstLetterHint} onClick={onFirstLetterHint} sx={{ width: 36, height: 36, borderRadius: 1.5, color: firstLetterHint ? "primary.contrastText" : "text.secondary", bgcolor: firstLetterHint ? "primary.main" : "transparent", "&:hover": { bgcolor: firstLetterHint ? "primary.dark" : "action.hover" } }}><InitialHintIcon active={firstLetterHint} /></IconButton></Tooltip><Tooltip title={copy.pauseAfterCorrectDescription}><IconButton size="small" aria-label={copy.pauseAfterCorrect} aria-pressed={pauseAfterCorrect} onClick={onPauseAfterCorrect} sx={{ width: 36, height: 36, borderRadius: 1.5, color: pauseAfterCorrect ? "primary.contrastText" : "text.secondary", bgcolor: pauseAfterCorrect ? "primary.main" : "transparent", "&:hover": { bgcolor: pauseAfterCorrect ? "primary.dark" : "action.hover" } }}><PauseCircleOutlineRoundedIcon fontSize="small" /></IconButton></Tooltip></>;
}

function AutoSpeakButton({ copy, active, onClick }: { copy: typeof nceeVocabularyTranslations.en; active: boolean; onClick: () => void }) {
  return <Tooltip title={copy.autoSpeakDescription}><IconButton size="small" aria-label={copy.autoSpeakLabel} aria-pressed={active} onClick={onClick} sx={{ width: 36, height: 36, borderRadius: 1.5, color: active ? "primary.contrastText" : "text.secondary", bgcolor: active ? "primary.main" : "transparent", "&:hover": { bgcolor: active ? "primary.dark" : "action.hover" } }}><VolumeUpRoundedIcon fontSize="small" /></IconButton></Tooltip>;
}
...
function WordMeaningAnswers({ exercise, values, onChange, onForget, onSubmit, inputRef, forgotten, copy }: { exercise: Exercise; values: string[]; onChange: (index: number, value: string) => void; onForget: () => void; onSubmit: () => void; inputRef: RefObject<HTMLInputElement | null>; forgotten: boolean; copy: typeof nceeVocabularyTranslations.en }) {
  const [showHint, setShowHint] = useState(false);
  const meanings = exerciseMeanings(exercise);
  const hints = exercise.meaningHints || [];
  return <Stack spacing={1}>
    {meanings.map((meaning, index) => <Stack key={`${meaning.partOfSpeech}-${index}`} direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ alignItems: { sm: "flex-start" } }}>
      <Chip size="small" color={meaningColors[meaning.partOfSpeech]} label={meaning.partOfSpeech} sx={{ mt: { sm: 1.5 }, minWidth: 48, alignSelf: { xs: "flex-start", sm: "auto" }, fontWeight: 800 }} />
      <TextField inputRef={index === 0 ? inputRef : undefined} autoFocus={index === 0} fullWidth multiline minRows={1} label={`${copy.answerMeaning} ${index + 1}`} value={values[index] || ""} onChange={event => onChange(index, event.target.value)} onKeyDown={event => { if (event.key === ";") { event.preventDefault(); onForget(); } else if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onSubmit(); } }} helperText={!forgotten && showHint && hints[index] ? hints[index] : undefined} sx={forgotten ? { bgcolor: "rgba(255, 193, 7, .18)" } : undefined} />
    </Stack>)}
    {!forgotten && hints.some(Boolean) && <Button size="small" sx={{ alignSelf: "flex-start" }} onClick={() => setShowHint(current => !current)}>{showHint ? copy.hideMeaningHint : copy.showMeaningHint}</Button>}
    {forgotten && <Box sx={{ px: 1.5, py: 1, borderLeft: 2, borderColor: "primary.main", bgcolor: "action.hover", borderRadius: 1 }}><Typography variant="caption" color="text.secondary">{copy.referenceMeanings}</Typography>{meanings.map((meaning, index) => <Typography key={index} variant="body2"><b>{meaning.partOfSpeech}</b> {meaning.text}</Typography>)}</Box>}
  </Stack>;
}
...
export default function NceeVocabularyClient() {
  const { locale } = useI18n(); const copy = nceeVocabularyTranslations[locale]; const settingsUrl = useSiteUrl("main", "/settings"); const { status, user } = useAuth(); useDocumentTitle(copy.title);
  ...
  const enrichWord = useCallback(async (word: Exercise) => {
    ...
    if (!word.quizDataset) return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };
    if (status === "uninitialized" || status === "authenticating" || status === "refreshing") return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };
    try {
      const cached = await vocabularyDrillApi.wordQuiz(word.quizDataset, word.id);
      if (cached.quiz) return { ...word, exerciseKind: cached.quiz.exerciseKind, meanings: cached.quiz.meanings.slice(0, 4), meaningHints: cached.quiz.hints.slice(0, 4) };
      if (!cached.canGenerate || !selected || !provider) return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };

      const response = await fetch(`/api/dictionary/oxford10c?word=${encodeURIComponent(word.english)}`);
      if (!response.ok) return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };
      ...
      if (!senses.length) return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };

      let exerciseKind: "meanings" | "dictionary" = "meanings"; let selectedSenseKeys: string[] = []; let completed = false;
      ...
      for await (const event of client.stream({
        ...
        systemPrompt: "你负责配置英语词义默写练习。若单词主要是基础功能词、语法词，或脱离上下文难以用少数中文大义项合理默写（例如 and、am、over），选择 dictionary，让学习者直接阅读词典。其他词选择 meanings，并从给定列表挑选 1 到 4 个现代、通用、彼此有实质区别的大义项；优先覆盖不同常用词性，排除罕见、古旧、方言、极专业和仅限固定短语的义项。不得改写或新增释义。必须调用 configure_word_exercise。",
        maxTokens: 512, temperature: .1, isComplete: () => completed, incompletePrompt: "立即调用 configure_word_exercise。", maxIncompleteRetries: 1,
        tools: [{ name: "configure_word_exercise", description: "选择词义默写或直接阅读词典，并配置最多四个大义项", parameters: { type: "object", properties: { kind: { type: "string", enum: ["meanings", "dictionary"] }, senseKeys: { type: "array", maxItems: Math.min(4, senses.length), uniqueItems: true, items: { type: "string", enum: senses.map(sense => sense.key) } } }, required: ["kind", "senseKeys"], additionalProperties: false }, execute: args => { const kind = args.kind === "dictionary" ? "dictionary" : "meanings"; const keys = Array.isArray(args.senseKeys) ? args.senseKeys.map(String) : []; const valid = kind === "dictionary" ? keys.length === 0 : keys.length >= 1 && keys.length <= 4; if (valid) { exerciseKind = kind; selectedSenseKeys = keys; completed = true; } return { accepted: valid }; } }],
      ...
      if (!completed) return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) };
      const saved = await vocabularyDrillApi.cacheWordQuiz({ dataset: word.quizDataset, sourceWordId: word.id, word: word.english, exerciseKind, senseKeys: selectedSenseKeys, generatorModel: selected.modelId });
      return { ...word, exerciseKind: saved.quiz?.exerciseKind || exerciseKind, meanings: saved.quiz?.meanings.slice(0, 4) || [], meaningHints: saved.quiz?.hints.slice(0, 4) };
    } catch { return { ...word, exerciseKind: "meanings" as const, meanings: fallback.slice(0, 4) }; }
  ...
  const loadMode = useCallback(async (targetDataset: string, targetMode: Mode, reset = false, forceReorder = reset) => { const sequence = ++loadSequenceRef.current; loadingRef.current = true; pendingBucketRef.current = null; refillInFlightRef.current = false; setLoading(true); setExercise(null); setPosition(0); setTotal(0); setBucketRemaining(0); setError(""); setGrade(null); setPhoneticRevealed(false); setForgotten(false); setWrongFlash(false); setAnswerCorrect(false); setAnswer(""); setMeaningAnswers([]); try {
    ...
    let orderGeneratedAt: string | undefined;
    let ids: number[]; if (targetDataset.startsWith("collection:")) {
      if (forceReorder) await vocabularyDrillApi.reorderCollection(targetDataset.slice(11));
      const data = await vocabularyDrillApi.userData();
      if (sequence !== loadSequenceRef.current) return;
      collectionsRef.current = data.collections; setCollections(data.collections);
      const collection = data.collections.find(item => targetDataset === `collection:${item.collectionId}`);
      if (!collection) throw new Error("collection_not_found");
      ids = collection.items.map(item => Number(item.sourceWordId)); orderGeneratedAt = collection.orderGeneratedAt;
    } else { const catalogResponse = await fetch(`/api/vocabulary-drill?dataset=${targetDataset}&mode=${targetMode}&catalog=1`, { cache: "no-store" }); if (!catalogResponse.ok) throw new Error(); ids = (await catalogResponse.json()).ids.map(Number); }
    if (!ids.length) throw new Error("empty_dataset");
    const savedProgress = !reset && progress && (targetDataset.startsWith("collection:") ? progress.orderGeneratedAt === orderGeneratedAt : isSameCatalog(progress.order, ids)) ? progress : null;
    const currentItemIndex = savedProgress && targetDataset.startsWith("collection:") ? ids.indexOf(savedProgress.order[savedProgress.index]) : -1;
    const savedIndex = savedProgress ? Math.min(Math.max(currentItemIndex >= 0 ? currentItemIndex : savedProgress.index, 0), ids.length - 1) : 0;
    progress = savedProgress
      ? { order: targetDataset.startsWith("collection:") ? ids : isCatalogOrder(savedProgress.order, ids) ? [...ids.slice(0, savedIndex), ...shuffled(ids.slice(savedIndex))] : savedProgress.order, index: savedIndex }
  ...
  function queueCollectionStat(kind: "appearances" | "correct" | "wrong") {
    ...
    if (kind !== "appearances") {
      const delta = deltas.find(item => item.collectionId === collectionId && item.dataset === sourceDataset && item.sourceWordId === exercise.id)!;
      delta.lastReviewedAt = new Date().toISOString();
      delta.reviews = [...(delta.reviews || []), { id: crypto.randomUUID(), reviewedAt: delta.lastReviewedAt, rating: kind === "wrong" ? "again" : phoneticRevealed ? "hard" : "good" }];
    }
  ...
  function recordWrong() { if (outcomeRecordedRef.current) return; outcomeRecordedRef.current = true; queueCollectionStat("wrong"); }
  ...
  useEffect(() => {
    ...
    const questionKey = `${loadSequenceRef.current}:${dataset}:${mode}:${position}:${exercise.sourceDataset || "custom"}:${exercise.id}`;
  ...
  async function checkAnswer() {
    ...
    if (!exercise) return;
    if (mode === "word" && exercise.exerciseKind === "dictionary") { await nextQuestion(); return; }
    const expectedMeanings = exerciseMeanings(exercise);
    const submittedMeanings = expectedMeanings.map((_meaning, index) => meaningAnswers[index]?.trim() || "");
    if (mode === "word" ? submittedMeanings.some(value => !value) : !answer.trim()) return;
    if (mode !== "word") { const isCorrect = mode === "meaning" ? lettersOnly(answer) === lettersOnly(exercise.english) : normalize(answer) === normalize(exercise.english); if (isCorrect) { recordCorrect(); if (mode === "meaning" && pauseAfterCorrect) setAnswerCorrect(true); else await nextQuestion(); } else { recordWrong(); setWrongFlash(false); window.requestAnimationFrame(() => { setWrongFlash(true); window.setTimeout(() => setWrongFlash(false), 650); }); } return; }
    ...
    try {
      ...
      for await (const event of client.stream({
        messages: [{ role: "user", content: JSON.stringify({ englishWord: exercise.english, dictionaryMeanings: expectedMeanings, learnerMeanings: submittedMeanings }) }],
        systemPrompt: "你负责批改高考英语多义项默写。dictionaryMeanings 与 learnerMeanings 按下标一一对应；必须逐项判断学习者写出的中文是否表达了对应词典义项的实质含义，允许自然的近义说法，但不能用另一个义项顶替。只有全部义项正确时 is_correct 才为 true；否则用简体中文简短指出错误的编号。必须且只能调用一次 grade_meaning。",
      ...
      if (completed.isCorrect) { recordCorrect(); setLoading(false); await nextQuestion(); } else { recordWrong(); setGrade(null); setWrongFlash(false); window.requestAnimationFrame(() => { setWrongFlash(true); window.setTimeout(() => setWrongFlash(false), 650); }); }
  ...
  async function forgetAnswer() {
    if (!exercise || loading || (mode === "word" && exercise.exerciseKind === "dictionary")) return; recordWrong(); setForgotten(true); setGrade(null); if (mode === "word") setMeaningAnswers(exerciseMeanings(exercise).map(meaning => meaning.text)); else setAnswer(exercise.english);
    ...
    } catch { setToast(copy.answerSaveFailed); }
  }
  ...
  async function deleteFromCollection() {
    ...
    try {
      ...
      const updatedCollections = collections.map(item => item.collectionId === collection.collectionId ? { ...item, items: remainingItems } : item); collectionsRef.current = updatedCollections; setCollections(updatedCollections); setToast(copy.removedFromCollection);
    ...
    catch { setToast(copy.removeFailed); } finally { setDeleting(false); }
  }
  ...
  async function deleteSelectedCollection() {
    ...
    try {
      ...
      setToast(copy.collectionDeleted);
    } catch { setToast(copy.collectionDeleteFailed); }
  ...
  async function transferCollectionItem() {
    ...
    try {
      ...
      setToast(result.restored ? copy.restoredToOriginal : copy.movedToTransferred);
      if (!remaining.length) setDataset(`collection:${result.destinationCollectionId}`); else await loadMode(dataset, mode);
    } catch { setToast(copy.transferFailed); }
  ...
  async function uploadProgress() {
    ...
    try { await vocabularyDrillApi.saveProgress({ dataset, mode, order: progress.order, index: progress.index, orderGeneratedAt: progress.orderGeneratedAt }); setSyncOpen(false); setToast(copy.cloudSaved); }
  ...
  async function downloadProgress() {
    ...
    try { const data = await vocabularyDrillApi.userData(); const progress = data.progress.find(item => item.dataset === dataset && item.mode === mode); if (!progress) { setSyncError(copy.noCloudProgress); return; }
      localStorage.setItem(`${PROGRESS_PREFIX}${dataset}:${mode}`, JSON.stringify({ order: progress.order, index: progress.index, orderGeneratedAt: progress.orderGeneratedAt })); setSyncOpen(false); await loadMode(dataset, mode); setToast(copy.cloudProgressDownloaded);
    } catch { setSyncError(copy.syncFailed); } finally { setSyncBusy(false); }
  }
  ...
  return <div className="page-below-navbar flex flex-col"><Box component="main" sx={{ flex: 1, px: { xs: 2, md: 3 }, py: 5 }}><Box sx={{ maxWidth: 780, mx: "auto" }}>
    <Card variant="outlined" sx={{ borderRadius: 3, overflow: mode === "meaning" ? "visible" : "hidden" }}><CardContent><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "minmax(0,1fr) auto" }, gap: 1.25, alignItems: "center" }}><Autocomplete size="small" options={datasets} value={datasets.find((item) => item.id === dataset) || builtInDatasets[0]} disableClearable getOptionLabel={(item) => item.name} onChange={(_event, value) => setDataset(value.id)} renderInput={(params) => <TextField {...params} label={copy.dataset} slotProps={{ ...params.slotProps, htmlInput: { ...params.slotProps.htmlInput, readOnly: true, sx: { userSelect: "none", cursor: "pointer" } } }} />} /><Stack direction="row" spacing={.5} sx={{ justifyContent: { xs: "flex-start", md: "flex-end" } }}><Tooltip title={copy.addCustomWord}><span><IconButton size="small" aria-label={copy.addCustomWord} disabled={status !== "authenticated" || user?.status !== 1} onClick={() => setCustomWordOpen(true)}><AddRoundedIcon fontSize="small" /></IconButton></span></Tooltip><Tooltip title={copy.createCollection}><span><IconButton size="small" aria-label={copy.createCollection} disabled={status !== "authenticated" || user?.status !== 1} onClick={() => { setResumeFavoriteAfterCollectionCreate(false); setCollectionDialogOpen(true); }}><CreateNewFolderRoundedIcon fontSize="small" /></IconButton></span></Tooltip>{selectedCollection && <Tooltip title={copy.deleteCurrentCollection}><IconButton size="small" color="error" aria-label={copy.deleteCurrentCollection} onClick={() => setCollectionDeleteOpen(true)}><DeleteForeverRoundedIcon fontSize="small" /></IconButton></Tooltip>}</Stack></Box></CardContent><Tabs value={mode} onChange={(_event, value: Mode) => { if (value === "phonetic") choosePhoneticTabGlyph(); setMode(value); window.location.hash = value; }} variant="fullWidth"><Tab value="phonetic" aria-label={copy.phonetic} label={<Tooltip title={copy.phonetic}><Box><ModeLabel from={phoneticTabGlyph} to="A" /></Box></Tooltip>} /><Tab value="meaning" aria-label={copy.meaning} label={<Tooltip title={copy.meaning}><Box><ModeLabel from="文" to="A" /></Box></Tooltip>} /><Tab value="word" aria-label={copy.word} label={<Tooltip title={copy.word}><Box><ModeLabel from="A" to="文" /></Box></Tooltip>} /></Tabs><CardContent><Stack spacing={2.25}>
      ...
      {mode === "word" && exercise?.exerciseKind !== "dictionary" && (!selected || !provider) && <Alert severity="info" action={<Button component={Link} href={settingsUrl}>{copy.settings}</Button>}>{copy.llmNeeded}</Alert>}
      ...
      {loading && !exercise ? <Box sx={{ display: "grid", placeItems: "center", py: 7 }}><CircularProgress /></Box> : exercise && <>
        ...
        {exercise.exerciseKind === "dictionary" && mode === "word" ? <Tooltip title={`${copy.next} (Ctrl + J)`}><Button fullWidth size="large" variant="contained" endIcon={<NavigateNextRoundedIcon />} onClick={() => void nextQuestion()}>{copy.next}</Button></Tooltip> : forgotten || answerCorrect ? <Tooltip title={`${copy.next} (Ctrl + J)`}><Button fullWidth size="large" variant="contained" color={answerCorrect ? "success" : "primary"} endIcon={<NavigateNextRoundedIcon />} onClick={() => void nextQuestion()}>{copy.next}</Button></Tooltip> : !grade ? <Tooltip title={mode === "word" ? copy.check : `${copy.check} (Enter)`}><Box component="span" sx={{ display: "flex", justifyContent: "center" }}><Button size="large" variant="contained" disabled={loading || (mode === "word" ? exerciseMeanings(exercise).some((_meaning, index) => !meaningAnswers[index]?.trim()) || !selected || !provider : !answer.trim())} onClick={() => void checkAnswer()}>{loading ? copy.checking : copy.check}</Button></Box></Tooltip> : <><Alert severity="warning">{grade.feedback}</Alert><Tooltip title={`${copy.next} (Ctrl + J)`}><Button fullWidth size="large" variant="contained" endIcon={<NavigateNextRoundedIcon />} onClick={() => void nextQuestion()}>{copy.next}</Button></Tooltip></>}
      </>}
    ...
    <VocabularyItemDialog open={customWordOpen} locale={locale} collections={collections} initialCollectionId={dataset.startsWith("collection:") ? dataset.slice(11) : collectionId} onClose={() => setCustomWordOpen(false)} onCollectionCreated={collection => { setCollections(current => [...current, collection]); setCollectionId(collection.collectionId); }} onSaved={(targetCollectionId, item) => { setCollections(current => current.map(collection => collection.collectionId === targetCollectionId ? { ...collection, items: [...collection.items, item] } : collection)); setCollectionId(targetCollectionId); setToast(copy.customWordSaved); }} />
  </Box></Box><Footer />
  <Dialog open={dictionaryOpen} onClose={() => setDictionaryOpen(false)} fullWidth maxWidth="lg" slotProps={{ paper: { sx: { height: { xs: "92dvh", sm: "82dvh" }, borderRadius: { xs: 2, sm: 3 } } } }}>
    <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1.25, pr: 1 }}><MenuBookRoundedIcon color="primary" /><Box sx={{ flex: 1 }}><Typography component="span" variant="h6" sx={{ fontWeight: 700 }}>{copy.dictionary}</Typography>{exercise && <Typography component="span" color="text.secondary" sx={{ ml: 1 }}>· {exercise.english}</Typography>}</Box><IconButton aria-label={copy.closeDictionary} onClick={() => setDictionaryOpen(false)}><CloseRoundedIcon /></IconButton></DialogTitle>
  ...
  <Dialog open={collectionDeleteOpen} onClose={deletingCollection ? undefined : () => setCollectionDeleteOpen(false)} fullWidth maxWidth="xs"><DialogTitle>{copy.deleteCollection}</DialogTitle><DialogContent><Alert severity="warning">{copy.deleteCollectionConfirmation(selectedCollection?.name || "")}</Alert></DialogContent><DialogActions><Button disabled={deletingCollection} onClick={() => setCollectionDeleteOpen(false)}>{copy.cancel}</Button><Button color="error" variant="contained" disabled={deletingCollection || !selectedCollection} onClick={() => void deleteSelectedCollection()}>{deletingCollection ? <CircularProgress size={20} color="inherit" /> : copy.delete}</Button></DialogActions></Dialog>
