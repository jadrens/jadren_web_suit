...
import { Alert, Autocomplete, Box, Button, Card, CardContent, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControlLabel, IconButton, LinearProgress, Stack, Switch, Tab, Tabs, TextField, Tooltip, Typography } from "@mui/material";
...
interface Exercise { id: number; english: string; phonetic: string; chinese: string; hintIndexes: number[]; duplicateCount: number; meanings?: DrillMeaning[]; sourceDataset?: string }
...
export default function NceeVocabularyClient() {
  ...
  async function saveFavorite() {
    ...
    try {
      setFavoriteOpen(false); void vocabularyDrillApi.addItem({ collectionId, dataset: exercise.sourceDataset || dataset, sourceWordId: exercise.id, word: exercise.english, phonetic: exercise.phonetic, meanings: exercise.meanings || [{ text: exercise.chinese, partOfSpeech: "other" }] }).then(() => setToast(copy.saved)).catch(() => undefined);
  ...
  return <div className="page-below-navbar flex flex-col"><Box component="main" sx={{ flex: 1, px: { xs: 2, md: 3 }, py: 5 }}><Box sx={{ maxWidth: 780, mx: "auto" }}>
    ...
    <Card variant="outlined" sx={{ borderRadius: 3 }}><CardContent><Stack spacing={1.25}><Autocomplete size="small" options={datasets} value={datasets.find((item) => item.id === dataset) || builtInDatasets[0]} disableClearable getOptionLabel={(item) => item.name} onChange={(_event, value) => setDataset(value.id)} renderInput={(params) => <TextField {...params} label={copy.dataset} slotProps={{ ...params.slotProps, htmlInput: { ...params.slotProps.htmlInput, readOnly: true, sx: { userSelect: "none", cursor: "pointer" } } }} />} /><Button sx={{ alignSelf: "flex-end" }} startIcon={<AddRoundedIcon />} disabled={status !== "authenticated" || user?.status !== 1} onClick={() => setCustomWordOpen(true)}>{locale === "zh" ? "新增自定义词汇" : "Add custom word"}</Button></Stack></CardContent><Tabs value={mode} onChange={(_event, value: Mode) => { setMode(value); window.location.hash = value; }} variant="fullWidth"><Tab value="phonetic" label={copy.phonetic} /><Tab value="meaning" label={copy.meaning} /><Tab value="word" label={copy.word} /></Tabs><CardContent><Stack spacing={2.25}>
      ...
      {mode === "phonetic" && <FormControlLabel control={<Switch checked={autoSpeak} onChange={(event) => { const checked = event.target.checked; setAutoSpeak(checked); localStorage.setItem(AUTO_SPEAK_KEY, String(checked)); }} />} label={copy.autoSpeak} />}
      {mode === "meaning" && <Stack direction={{ xs: "column", sm: "row" }} spacing={{ sm: 2 }}><FormControlLabel control={<Switch checked={firstLetterHint} onChange={(event) => { const checked = event.target.checked; setFirstLetterHint(checked); localStorage.setItem(FIRST_LETTER_HINT_KEY, String(checked)); }} />} label={locale === "zh" ? "首个提示固定为首字母" : "Always hint the first letter first"} /><FormControlLabel control={<Switch checked={pauseAfterCorrect} onChange={(event) => { const checked = event.target.checked; setPauseAfterCorrect(checked); localStorage.setItem(PAUSE_AFTER_CORRECT_KEY, String(checked)); }} />} label={locale === "zh" ? "答对后停留" : "Pause after a correct answer"} /></Stack>}
